<?php
    ...
    $query = "select * from chat_message where (to_user_id = $to_user_id and from_user_id = $from_user_id) or (to_user_id = $from_user_id and from_user_id = $to_user_id) order by time_stamp ";
    $result = mysqli_query($con, $query);
    ...
    else
    {
        ...
        foreach($result as $row)
        {
            ...
            if(!is_null($row['chat_media']))
            {
                ...
                if(strcmp($file_extension,"MOV") == 0)
                {
                ...
                elseif(strcmp($file_extension,"jpeg") == 0)
                {   
                    $media = "<div style='clear: both'><img src='media/".$row['chat_media']."' height='250px' width='250px'></div>";
                }
                elseif(strcmp($file_extension,"jpg") == 0)
                {   
                    $media = "<div style='clear: both'><img src='media/".$row['chat_media']."' height='250px' width='250px'></div>";
                }
                elseif(strcmp($file_extension,"png") == 0)
                {   
                    $media = "<div style='clear: both'><img src='media/".$row['chat_media']."' height='250px' width='250px'></div>";
                }
            }
            ...
            else
            {
                ...
                {
                    $content = "<li class='to-left' style='clear: both; float: left;'>"
                                .$media."<span style='clear: both; float: left;' >"
                                .$message."</span><br>
                                <small style='clear: both; float: right; background-color: #E1F3FA; padding: 3px; border-radius: 5px;'><em>".$row['time_stamp']."</em></small></li>";
                }
